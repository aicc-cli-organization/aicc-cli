    ...
    <div class="count-item">
      ...
      <ul>
        <li v-for="(duration, index) in durationList" :key="index" :style="{'flex-basis': duration['flex-basis']}">
          <div class="box" :class="duration.isEmpty ? 'empty' : '' " :style="{'background-color': duration['background-color'] }">{{duration.boxText}}</div>
          <span class="num" v-if="!duration.isEmpty">{{duration.num || 0}}通</span>
        </li>
      </ul>
    </div>
    <div class="count-item last">
      <div class="title">对话轮次</div>
      <ul>
        <li v-for="(chatRound, index) in chatRoundList" :key="index" :style="{'flex-basis': chatRound['flex-basis']}">
          <div class="box" :class="chatRound.isEmpty ? 'empty' : '' " :style="{'background-color': chatRound['background-color']}">{{chatRound.boxText}}</div>
...
export default {
  ...
  computed: { 
    durationList() {
      ...
      return this.getDurationPercentList(chatDuration, overviewDuration)
    },
    chatRoundList() {
      const { chatRound = {} } = this.fullStatsInfo
      return this.getDurationPercentList(chatRound, overviewChatRound)
    },
  },
  methods: {
    getDurationPercentList(duration, configData) {
      const deepConfigData = JSON.parse(JSON.stringify(configData))
      let durationTotal = 0
      for (let key in duration) {
        durationTotal += (duration[key] || 0)
      }
      const list = []
      for (let item of deepConfigData) {
