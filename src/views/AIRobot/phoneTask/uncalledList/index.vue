<template>
  <div id="record-detail">
      ...
      <div class="action-right">
        <div class="right">
          ...
          <!-- <el-button class="right white-list-btn" :disabled="!hasAccess('ADMIN_TASK_WHITE_LIST')">
            <router-link :to="{ name: 'whiteList'}">客户白名单</router-link>
          </el-button> -->
          <el-button type="primary" class="right-item" v-if="hasAccess('crm_callTask_exportCus')" @click="exportCustomerPerson">导出客户</el-button>
    ...
    >
      ...
      <el-table-column prop="calledPhoneNumber" label="联系电话"></el-table-column>
      <el-table-column prop="customerPersonGroupName" label="客户分组"></el-table-column>
      ...
      <el-table-column prop="status" label="执行状态">
        <template slot-scope="scope">{{parseStatus(scope.row.status)}}</template>
      </el-table-column>
      <el-table-column fixed="right" label="操作" width="80">
        <template slot-scope="scope">
          <YiwisePopConfirm label="确认删除该客户吗？" @submit="removeCustomerFromRobotCallJob(scope.row)">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      parseStatus(status) {
        return constant.taskStatus[status]
      },
