 ...
 *   <div id="zoomSliderNumber">100%</div>
 * </div>
 * ```
 *
 * <p class="box">
...
ZoomSlider.prototype.sliderDivSetup = function() {
  ...
  zoomOutBtn.innerHTML = '<i class="el-icon-minus"></i>';
  ...
  zoomInBtn.innerHTML = '<i class="iconfont icon-tianjiajiahaowubiankuang"></i>';
  this._sliderDiv.appendChild(zoomInBtn);

  var zoomSliderNumber = document.createElement('div')
  zoomSliderNumber.id = 'zoomSliderNumber';
  zoomSliderNumber.innerHTML = '100%';
  this._sliderDiv.appendChild(zoomSliderNumber);
  ...
  if (this._diagramDiv !== null) {
    ...
    this._diagramDiv.appendChild(this._sliderDiv)
...
ZoomSlider.prototype.resize = function(reorient) {
  ...
  this._sliderDiv.style.zIndex = '500';
  this._sliderDiv.style.position = 'absolute';
...
ZoomSlider.prototype.reorient = function() {
  var zoomOutBtn = document.getElementById('zoomSliderOut');
  var zoomInBtn = document.getElementById('zoomSliderIn');
  var zoomSliderNumber = document.getElementById('zoomSliderNumber')
  var zoomRangeInput = document.getElementById('zoomSliderRange');
  ...
  if (this.orientation === 'horizontal') {
    ...
    this._sliderDiv.insertBefore(zoomInBtn, zoomSliderNumber);
    // this._sliderDiv.appendChild(zoomInBtn);
  } else {
...
ZoomSlider.prototype.scaleToValue = function() {
  ...
  slider.value = (y1 / A) * 100// Math.round(Math.log(y1/A) / Math.log(B)).toString();
  zoomSliderNumber.innerHTML = ((y1 / A) * 100).toFixed(0) + '%'
}
...
ZoomSlider.prototype.valueToScale = function() {
  ...
  var A = this._initialScale;
  // var B = diagram.commandHandler.zoomFactor;
  diagram.scale = A * (x / 100) // Math.pow(B, x);
  zoomSliderNumber.innerHTML = x.toFixed(0) + '%'
}
