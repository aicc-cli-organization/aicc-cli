  ...
  <div class="task-list-wrap">
    ...
    <ul class="task-list" v-if="totalElements > 0">
      <li class="task-item" @click="onSelectTask(taskItem)"
        ...
        <task-reason
          :hangUpReason="taskItem.hangUpReason"
          :taskStatus="taskItem.status"
          :nextStartTime="taskItem.nextStartTime"
          :priority="taskItem.priority"
        >
        </task-reason>
        <div class="progress">
        ...
        <div v-if="isAdmin">
          是否使用一知ASR: {{taskItem.useYiwiseAsr ? '是' : '否'}}
        </div>
...
import { getDialogFlowExtInfo } from '@/api/talk'
import { EDIT_TYPE_LIST } from '../config/enum'
...
export default {
  ...
  methods: {
    ...
    async onSelectTask(robotCallJob) {
      ...
      const { data } = await getDialogFlowExtInfo({
        dialogFlowId: robotCallJob.dialogFlowId
      })

      this.$store.commit('UPDATE_HUMAN_SERVICE', data)
    },
    onEditTask(taskId) {
